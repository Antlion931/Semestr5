...
pub fn ast_to_pre_assembler(ast: AST) -> Block {
    ...
    for d in ast.main.declarations {
        match d.decl_type {
            DeclType::Number => {
                result.memory.insert(AbstractVarible::Else(AbstractNumber::Var(d.name)), index);
                index += 1;
            },
            DeclType::Table(x) => {
                for i in 0..x {
                    result.memory.insert(AbstractVarible::Table(d.name.clone(), AbstractNumber::Const(i)), index);
                    index += 1;
                }
            },
        }
    ...
    for com in ast.main.commands {
        match com {
            ...
            Command::Read(x) => {
                ...
                result.pre_assembler.push(PreAssembler::PUT(identifier_to_abstract_varible(x.identifier)));
            },
            Command::Write(x) => {
                result.pre_assembler.push(PreAssembler::GET(value_to_abstract_varible(x.value)));
...
fn identifier_to_abstract_varible(id: Identifier) -> AbstractVarible {
    match id.indentifier_type {
        ...
        IdentifierType::TableWithNumber(x) => AbstractVarible::Table(id.name, AbstractNumber::Const(x)),
        IdentifierType::TableWithIdentifier(x) => AbstractVarible::Table(id.name, AbstractNumber::Var(x)),
    }
}
