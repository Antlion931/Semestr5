...
// O(c*p^2), where  p = length(p), c = length(c), but in most cases it is much faster.
pub fn pattern_finder(pattern: &str, content: &str) -> Vec<usize> {
    ...
    for (i, cc) in c.iter().enumerate() {
        // c times
        q = longest_prefix_of_a_that_is_suffix_of_b(
            // once for every input pair O(p^2), then const
            pattern,
            (pattern_graphemes[..q].iter().copied().collect::<String>() + cc).as_str(),
        );
        if q == m {
            result.push(i + 1 - m);
        }
...
#[cached(
    key = "String",
    convert = r#"{ format!("{}.{}{}", a_str.len(), a_str, b_str) }"# // good key as first number
                                                                     // say how long pattern is, so
                                                                     // the only way to produce the
                                                                     // same key, is to have the
                                                                     // same a_str and b_str
)]
